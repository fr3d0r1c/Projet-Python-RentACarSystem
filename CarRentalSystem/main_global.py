...
# On importe votre UI existante pour réutiliser la gestion de flotte
...
def main():
    ...
    # 2. Chargement des données (Flotte uniquement pour l'instant via StorageManager)
    # Pour le prototype, on charge la flotte via le storage existant
    # Plus tard, le StorageManager devra tout gérer
    ...
    while True:
        ...
        # ---------------------------------------------------------
        # 1. GESTION DE LA FLOTTE (Votre ancien menu)
        # ---------------------------------------------------------
        if choice == "1":
            while True:
                # On utilise vos fonctions existantes de console_ui !
                # On fait une mini-boucle pour rester dans le menu flotte
                print("\n")
                console.rule("[bold cyan]GARAGE & ÉCURIE[/]")
                fleet_ui.show_main_menu() # Affiche votre menu existant
                
                # On ajoute une option de retour explicite
                ...
                if sub_choice == '0': break
                    ...
                    # Si vous avez ajouté Stats & Recherche en 7
